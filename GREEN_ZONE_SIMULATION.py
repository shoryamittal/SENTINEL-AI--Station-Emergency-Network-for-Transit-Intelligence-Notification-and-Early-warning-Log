#!/usr/bin/env python3
"""
SENTINEL AI - Green Zone Simulation
Demonstrates normal crowd conditions
"""
import cv2
import numpy as np
import time
...
def main():
    logger.info("=" * 80)
    logger.info("🟢 SENTINEL AI - Green Zone Simulation")
    logger.info("=" * 80)
    ...
    try:
        while True:
            # Generate green zone density (2.0-3.5 ppl/m²)
            density = 2.0 + np.random.uniform(0.5, 3.5)
            ...
            state, confidence = classifier.classify(
                statistics["max_density"], "stable", prediction)
            ...
            cv2.putText(vis_frame, "GREEN ZONE - NORMAL CROWD", (10, 30),
            ...
            cv2.putText(vis_frame, f"DENSITY: {statistics['max_density']:.2f} ppl/m²", (10, 90),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2)
            cv2.imshow("Green Zone Simulation", vis_frame)
        ...
        logger.info("=" * 80)
        logger.info("✅ Green Zone Simulation Complete")
        logger.info("=" * 80)
